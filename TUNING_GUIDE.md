...
| The explanation LLM's tone/style | `.env`'s `WEATHERGPT_EXPLANATION_TONE`, or the grounding rules themselves in `app/agents/orchestrator.py` — `_EXPLANATION_SYSTEM` |
...
Run `pytest -q` after any change above — 256+ tests, ~6s.
